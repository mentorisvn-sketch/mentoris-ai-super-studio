...
export const AuthPage = ({ onBackToHome }: { onBackToHome: () => void }) => {
  // ✅ Lấy thêm setViewMode và refreshUser để chuyển trang sau khi login
  const { supabase, setViewMode, refreshUser } = useApp();
  ...
  const handleAuth = async (e: React.FormEvent) => {
     ...
     try {
        if (isLoginMode) {
            ...
            // ✅ Fix: Load lại user và chuyển ngay vào Studio
            await refreshUser();
            setViewMode('studio'); 
            ...
            if (error) throw error;
            toast.success("Đăng ký thành công!", { description: "Vui lòng kiểm tra email xác nhận hoặc đăng nhập ngay." });
            setIsLoginMode(true);
        }
     } catch (err: any) {
         const errorMsg = err.message === "Invalid login credentials" 
            ...
            : err.message;
         setError(errorMsg);
         toast.error("Thất bại", { description: errorMsg });
     } finally {
         setLoading(false);
     }
  };

  // ✅ LOGIC GOOGLE LOGIN MỚI (Đã thay thế code cũ)
  const handleGoogleLogin = async () => {
      try {
          const { error } = await supabase.auth.signInWithOAuth({
              provider: 'google',
              options: {
                  redirectTo: window.location.origin // Quay về trang chủ sau khi xong
              }
          });
          if (error) throw error;
      } catch (error: any) {
          toast.error("Lỗi đăng nhập Google", { description: error.message });
      }
  };
